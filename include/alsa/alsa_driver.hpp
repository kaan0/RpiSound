...
public:
    ...
    // Audio operations
    Result<size_t> write(const HandlePtr& handle, const types::AudioDeviceInfo& deviceInfo, const types::audio_span_t& audioData) override;
    Result<size_t> read(const HandlePtr& handle, const types::AudioDeviceInfo& deviceInfo, types::audio_span_mut_t& audioBuffer, size_t framesToRead) override;
...
private:
    static AlsaHandle& ah(const HandlePtr& h) {
        return static_cast<AlsaHandle&>(*h);
    }
    static AlsaFacade::PcmHandle* ph(const HandlePtr& h) {
        return ah(h).p;
    }
};
